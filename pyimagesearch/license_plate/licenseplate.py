...
class LicensePlateDetector:
	...
	def detect(self):
		...
		for lpRegion in lpRegions:
			# detect character candiadates in the current license plate region
			lp = self.detectCharacterCandidates(lpRegion)

			# only continue if characters were succesfully detected
			if lp.success:
	...
	def detectCharacterCandidates(self, region):
		...
		# resie the license plate region to a canonical size
		plate = imutils.resize(plate, width=400)
		thresh = imutils.resize(thresh, width=400)
		cv2.imshow("Thresh", thresh)

		# perform connected components analysis and initialize the mask to store the locations
		...
		for label in np.unique(labels):
			# if this is a backaground label, ignore it
			if label == 0:
				continue
			...
			# ensure that atleast one contour was found in the mask
			if len(cnts) > 0:
				# grab the largest contour whcih corresponds to the component in the mask, then
